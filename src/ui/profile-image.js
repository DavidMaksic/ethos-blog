...
function ProfileImage({ user, image }) {
   ...
   return (
      <form
         className="size-fit flex flex-col items-center gap-4 bg-white dark:bg-primary-300/10 rounded-3xl border border-quaternary dark:border-tertiary text-lg text-[#4d525c] dark:text-slate-300/80 font-medium px-20 py-6 transition-200"
         action={action}
      >
         ...
         <label
            className="relative size-24 hover:opacity-80 cursor-pointer transition-[opacity_0.2s]"
            htmlFor="image"
         >
            <RemoteImage
               imageRef={imageRef}
            ...
            <LuPencil className="absolute right-0 bottom-0 size-7 px-[0.4rem] pt-px rounded-full text-primary-500/80 dark:text-primary-400 bg-white dark:bg-primary-200 border border-primary-300 dark:border-quaternary transition-200" />
         </label>
         ...
         >
            {isPending ? (
               <div className="relative">
                  <span className="opacity-0 text-xl">Update</span>
