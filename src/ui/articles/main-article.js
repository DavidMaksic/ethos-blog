...
function MainArticle({ article }) {
   ...
   return (
      ...
      >
         <div className="space-y-2 md:space-y-3 self-center flex flex-col  md:items-center md:order-2 md:w-full md:px-10 xs:px-8 md:bg-white md:dark:bg-primary-300/15 md:py-6 md:pb-7 md:rounded-3xl md:box-shadow md:border md:border-quaternary md:dark:border-primary-300/15">
            <h2 className="relative md:text-center md:font-semibold styled_text text-6xl xl:text-[3.5rem] lg:text-[2.8rem] md:text-[2.8rem] xs:leading-12.5 pb-2 lg:pb-0 bg-gradient-to-r from-gray-600 to-gray-500/90 dark:from-slate-300 dark:to-slate-300/80 select-none leading-16 lg:leading-13">
               {article.title}
            </h2>
            <h3
               className={`text-[1.6rem] lg:text-[1.2rem] md:text-[1.4rem] text-primary-400 xs:w-11/12 ${cormorantGaramond.className} select-none md:text-center`}
            >
               {article.description}
            </h3>
