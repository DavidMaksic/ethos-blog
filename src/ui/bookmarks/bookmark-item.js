...
function BookmarkItem({ article, categories }) {
   ...
   return (
      ...
      >
         <Link
            href={`/${article.id}`}
            className="relative grid grid-cols-[2fr_0.8fr] xs:grid-cols-[2fr_0.3fr] min-h-45.5 xl:min-h-40 lg:min-h-38 md:min-h-52 sm:min-h-[18vh] xs:min-h-[20vh] rounded-2xl group cursor-pointer overflow-hidden border border-quaternary dark:border-primary-300/15 bg-white dark:bg-primary-300/15 hover:translate-x-1.5 transition-[translate] duration-200 select-none box-shadow"
         >
            <div className="self-center py-2 px-12 xs:px-12 md:pr-0 space-y-5 xl:space-y-4 z-20">
               <h2
                  className={`text-primary-500 dark:text-primary-600/85 text-3xl lg:text-[2rem] xs:text-[1.85rem] font-medium dark:font-normal lg:font-semibold lg:dark:font-normal md:dark:font-medium lg:leading-10 xs:leading-9 ${
                     ...
                     'text-[1.8rem]! lg:text-[1.7rem]! md:text-[1.6rem]! sm:text-[1.4rem]! xs:text-[1.55rem]! leading-[2.5rem]! xl:leading-[2.4rem]! lg:leading-[2.2rem]! md:leading-[2.1rem]! sm:leading-[2rem]! xs:leading-[1.98rem]!'
                  }`}
               >
                  {article.title}
               </h2>
