...
function UserComment({ comment, users, user, articles, allComments }) {
   ...
   return (
      <Link
         href={fullUrl}
         className="flex flex-col gap-5 last:border-b-transparent border-b border-b-quaternary dark:border-b-primary-300/23 lg:dark:border-b-primary-300/28 hover:bg-primary-300/10 dark:hover:bg-primary-300/7 px-14 lg:px-12 pt-8 lg:pt-6 pb-11 lg:pb-8 scroll-mt-28! transition select-none cursor-pointer"
      >
         <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 lg:text-[1.1rem] md:text-xl">
               ...
               <span className="text-primary-400 lg:hidden md:block">•</span>
               <span className="font-thin text-primary-400 lg:hidden md:block">
                  {date}
               </span>
            </div>
         </div>
         ...
         <p
            className={`${ebGaramond.className} text-[1.35rem] lg:text-xl md:text-[1.62rem] whitespace-pre-line`}
         >
            {comment.content}
         </p>
      </Link>
